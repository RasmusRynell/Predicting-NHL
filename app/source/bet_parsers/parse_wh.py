...
def read_file(file):
    ...
    for match in matches:
        ...
        for i in range(0, len(matches[match]), 5):
            info = matches[match][i:(i+5)]
            if(len(info) > 1):
                player_name = info[0].split(", ")
                player_name_first = player_name[1].lower()
                player_name_last = player_name[0].lower()
                player_name = unidecode(player_name_first+" "+player_name_last)
                player_target = info[1].split(" ")[1]
                player_odds_O = info[2]
                player_odds_U = info[4]
